"""Browser support — serves the IIFE client and handles browser log ingest."""
from __future__ import annotations

# The browser IIFE is embedded as a string constant.
# Generated from packages/debugger/dist/browser/client.js
CLIENT_SCRIPT = r"""(() => {
...
SCRIPT_TAGS = '<script>window.__DEBUGGER_INGEST_URL__="/_/d";</script><script src="/_/d.js" defer></script>'


def inject_scripts(html: str) -> str:
    """Inject debugger script tags into HTML before </body>.

    If the tags are already present, the original HTML is returned unchanged.
    If there is no </body> tag, the original HTML is returned unchanged.

    Args:
        html: The HTML string to inject into.

    Returns:
        The modified HTML with script tags injected.
    """
    if "__DEBUGGER_INGEST_URL__" in html:
        return html
    idx = html.rfind("</body>")
    if idx == -1:
        return html
    return html[:idx] + SCRIPT_TAGS + html[idx:]
