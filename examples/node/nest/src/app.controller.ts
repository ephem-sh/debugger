...
<html lang="en">
<head><meta charset="utf-8"><title>NestJS — Debugger Test</title></head>
<body style="max-width:640px;margin:0 auto;padding:32px 16px;font-family:system-ui,sans-serif">
<h1 style="font-size:20px;font-weight:600;margin:0 0 24px">NestJS — Debugger Test</h1>
...
export class AppController {
  ...
  @Get('api/test')
  getTest(@Query('id') id: string, @Query('error') error: string, @Res() res: Response) {
    console.log('test endpoint', { id });
    if (error === 'true') {
      console.error('test error triggered', { id });
      res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ error: 'test error', id });
      return;
    }
    res.json({ ok: true, id });
  }
